...
def show_result_generator(loaded_model, batch_generator, num_plot=2, size=(28, 28),
	...
	for i in range(num_plot):
		batch_X, batch_y = next(batch_generator)  # **从迭代器获取下一批数据**
		...
		# 绘制三张图
		axes[i, 0].imshow(batch_X.reshape(batch_X.shape[0], row, col)[0], cmap='jet')
		axes[i, 0].set_title(title1)
		axes[i, 0].axis('off')
		...
		axes[i, 2].imshow(outputs_mask.numpy().reshape(batch_X.shape[0], row, col)[0], cmap='jet')
		axes[i, 2].set_title(title3)
		axes[i, 2].axis('off')
...
def plot_multiple_data(datas, titles=None, logic_function=None, data_single_scale=(28, 28), cols=4):
	...
	for i in range(num_samples):
		...
		ax.imshow(datas[i].reshape(data_single_scale), cmap="jet")  # 显示图像
		ax.axis("off")  # 关闭坐标轴
